function [S_fpj,A_ptj]=NMF_factorization(X_ft,NMFparams,NMF_inic)
%% Load Parameters
NUM_MAX_ITER = NMFparams.NUM_MAX_ITER;
B = NMFparams.B;
...
for ite = 1:NUM_MAX_ITER
    ...
    %% Reconstruction and Normalizations
    [Y_ft]=nmf_inc_generateY(NMFparams,Sn_fpj,An_ptj,eS_pj,eA_t);
    if (B==1)
        norma_Yft = (sum(sum(Y_ft.^B,2))).^(1/B);
    elseif (B==0)
    ...
    KL_num = permute(tensor_mult(Sne_fpj,((Y_fts_cb2 .* Xn_ft)* diag(eA_t)),1,1),[1 3 2]);
    KL_num = tprod(KL_num,[-1 2 -2], En_kpj, [1 -1 -2]);
    KL_den=permute(tensor_mult(Sne_fpj,(Y_fts_cb1* diag(eA_t)),1,1),[1 3 2]);
    KL_den = tprod(KL_den,[-1 2 -2], En_kpj, [1 -1 -2]);
...
S_fpj = Sn_fpj.*permute(repmat(eS_pj,1,1,f_max),[3 1 2]);
A_ptj = tprod(En_kpj,[-1 1 3],An_kt * diag(eA_t),[-1 2]);
