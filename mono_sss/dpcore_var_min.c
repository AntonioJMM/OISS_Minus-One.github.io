...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nlhs > 0) {
        ...
        int rows, cols, i, j, k, tb, posD_i, posD_j;
        double *pM, *pD, *pP, *pC;
        double d1, d2, d;
        double *costs;
        int *steps;
        ...
        for (j = 0; j < cols; ++j){
            for (i = 0; i < rows; ++i){
                ...
                for (k = 0; k < ncosts; ++k){
                    ...
                    if (i==0 && j==0){ /*Sin i==0 empieza en cualquier tiempo midi de la primera trama real*/
                        d2 = 0;
                        
                    /*}else if(j==0){
                        d2 = -INF; //Para empezar en esquina inferior izquierda
                        */
                    }else{
                        posD_i = i-steps[2*k];
                        posD_j = j-steps[2*k+1];
                        
                        if ( posD_i >= 0 && posD_j >= 0 ){
                            d2 = d1*costs[k] + pD[posD_i + posD_j*rows];
                        }else{
                            ++special;
                        }
                    }
